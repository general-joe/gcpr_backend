...
///Switching to Resend for email sending
...
/**
 * Send email immediately using Resend
 */
export async function sendEmailNow(to, templateName, variables = {}) {
  const fileName = templateMap[templateName];
  ...
  if (!mergedVariables.name) {
    mergedVariables.name =
      mergedVariables.fullName || "there";
  }
  ...
  try {
    ...
    const fromName =
      process.env.RESEND_FROM_NAME || "GetMyNeuroCare";
